function init_edit_buttons() {
  $('#hy-top a').button();
  $('.hy-el-menu a').each(function(idx, el){
...
jQuery(function($) {
  ...
  $('.hy-el').live('mouseover', function(){
    ...
    $('.hy-marker.lt', el).position({ my: 'left top', at: 'left top', of: el });
    $('.hy-marker.rt', el).position({ my: 'right top', at: 'right top', of: el });
    $('.hy-marker.rb', el).position({ my: 'right bottom', at: 'right bottom', of: el });
    $('.hy-marker.lb', el).position({ my: 'left bottom', at: 'left bottom', of: el });
  ...
  $('div[data-dropable]').css('min-height','20px');
  $('div[data-dropable]').sortable({
    revert: true,
    items: '> div[data-draggable]',
    handle: 'a[data-draghandle]',
    connectWith: 'div[data-dropable]',
    cursor: 'move'
  });
  
  $('div[data-addable]').draggable({
    connectToSortable: 'div[data-dropable]',
    helper: 'clone',
    revert: 'invalid'
  });
